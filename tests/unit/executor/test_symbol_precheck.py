...
class _PrecheckExecutor(AbstractExecutor):
    def __init__(self, decisions: dict[str, tuple[bool, str | None]]) -> None:
        ...
        super().__init__(
            TradeChannel.CTP,
            CTPAccountConfig(
                broker_id="b", investor_id="i", password="p", td_front="tcp://td:1", md_front="tcp://md:2", app_id="app", auth_code="auth"
            ),
        )
    ...
    def get_market_data(self, symbols: list[str]) -> dict[str, UnifiedPriceData]:
        ...
        return {
            symbol: UnifiedPriceData(symbol=symbol, last_price=100, bid_price=99, ask_price=101, bid_volume=1, ask_volume=1, volume=1, turnover=100, timestamp=1, update_time="2026-08-25T21:29:00")
    ...
    def _place_order_impl(self, symbol: str, direction: OrderDirection, order_type: OrderType, volume: float, price: float = 0, **kwargs: object) -> UnifiedOrder:
        _ = (direction, order_type, volume, price, kwargs)
        return UnifiedOrder(order_id=symbol, symbol=symbol, direction=OrderDirection.BUY, order_type=OrderType.LIMIT, volume=1, price=100, status="SUBMITTED")
...
def _input() -> UnifiedStandardInput:
    return UnifiedStandardInput(
        ...
        account_config=CTPAccountConfig(broker_id="b", investor_id="i", password="p", td_front="tcp://td:1", md_front="tcp://md:2", app_id="app", auth_code="auth"),
...
def test_all_symbol_prechecks_block_without_market_data_websocket_or_cancel() -> None:
    executor = _PrecheckExecutor({"IF2609": (False, "CTP.SESSION.CLOSED"), "ag2612": (False, "CTP.SESSION.SNAPSHOT_MISSING")})
