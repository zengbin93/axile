...
def test_chase_consistency():
    ...
    try:
        ...
        if "追单总时间" in error_msg or "过长" in error_msg:
            print(f"✓ 总追单时间验证通过: {error_msg}")
        else:
            print(f"⚠️  总追单时间验证错误消息不符合预期: {error_msg}")
    ...
    try:
        ...
        if "速率限制" in error_msg:
            print(f"✓ 速率限制验证通过: {error_msg}")
        else:
            print(f"⚠️  速率限制验证错误消息不符合预期: {error_msg}")
