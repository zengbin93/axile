...
def test_resolve_execution_backend_kind_defaults_by_channel() -> None:
    ...
    assert resolve_execution_backend_kind(TradeChannel.QMT) == ExecutionBackendKind.THREAD
    assert resolve_execution_backend_kind(TradeChannel.CTP) == ExecutionBackendKind.THREAD
