...
class QMTAccountConfig(BaseAccountConfig):
    """
    QMT 账户配置.
    ...
    账号: str = Field(..., description="QMT账户")
    密码: str = Field(..., description="密码")
    应用路径: str = Field(..., description="QMT应用路径")
    窗口标题: str = Field(..., description="QMT窗口标题")
    会话ID: int = Field(..., description="会话ID")
    数据路径: str = Field(..., description="QMT数据路径")
    ...
    def set_channel_type(cls, data: dict[str, object] | BaseAccountConfig) -> dict[str, object] | BaseAccountConfig:
        ...
        if not isinstance(data, dict):
            return data
        data["channel_type"] = TradeChannel.QMT
        return data

    @override
    def __str__(self) -> str:
        """返回便于记录日志的简要字符串表示."""
        return f"QMTAccountConfig(账号={self.账号})"
