...
def build_execution_result_details(result: Mapping[str, object]) -> dict[str, object]:
    """转发结果契约中的状态、错误及品种证据。"""
    return {key: result[key] for key in ("status", "error", "symbol_results") if key in result}
