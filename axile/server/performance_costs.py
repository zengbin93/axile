...
def execution_list_payload(payload: dict) -> dict:
    """裁剪新旧绩效投影的逐单证据，只保留列表状态和统计。"""
    record = mapping(payload.get("record"))
    raw = mapping(record.get("raw_result"))
    results = mapping(raw.get("symbol_results"))
    compact = {key: raw[key] for key in ("status", "task_status", "error", "msg", "execution_kind") if key in raw}
    if isinstance(raw.get("symbol_results"), dict):
        compact["symbol_results"] = {
            symbol: {key: result[key] for key in ("status", "error") if key in result}
...
def project_execution(record) -> tuple[dict, list[dict]]:
    ...
    payload = {
        ...
        "record": {
            ...
            "raw_result": {
                key: raw[key]
                for key in ("status", "task_status", "error", "msg", "execution_kind", "symbol_results")
                if key in raw
            },
        },
        "noop": noop,
