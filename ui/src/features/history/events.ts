...
export function performanceEvents(bindings: PortfolioAccountRecord[], activity: AccountActivity[], window: TimeWindow, names: Map<number, string>) {
  return [
    ...
    ...activity.flatMap(a => a.kind === 'schedule_skip' ? [{ time: a.occurred_at, tag: '跳过', text: a.reason_code === 'BUSY' ? '执行中' : '休市', executionId: null as string | null }] : a.record.is_success !== 1 ? [{ time: a.occurred_at, tag: a.record.task_status === 'TERMINATED' ? '终止' : '失败', text: executionRecordError(a.record) || '执行未完成', executionId: a.record.execution_id }] : []),
