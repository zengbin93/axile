...
function baseFixture(): { events: ExecutionEvent[]; artifacts: ExecutionArtifact[] } {
  const events: ExecutionEvent[] = [
    ...
    ev({ event_type: 'target_computed', seq: 3, details: { decision: { curr_target: { BTCUSDT: 0.9, ETHUSDT: 0.9 } } } }),
    ev({
  ...
  const artifacts: ExecutionArtifact[] = [
    art('standard_input', {
      input: {
        curr_target: { ETHUSDT: 0.9 },
        last_target: { ETHUSDT: 1.5 },
        algorithm: { method: 'SINGLE-MAKER', params: { price_strategy: 'PASSIVE', chase_enabled: true, max_chase_count: 50, chase_interval: 5 } },
        forbidden_symbols: [],
        risk_symbols: [],
      },
    }),
    art('target_snapshot', { curr_target: { ETHUSDT: 0.9 }, last_target: { ETHUSDT: 1.5 } }),
    art('account_snapshot_before', {
      account_assets: { total_asset: 5147.62, market_value: 15620.7, positions: [{ symbol: 'ETHUSDT', extra: { unrealized_pnl: 51.53 } }], source: 'real', update_time: '2026-07-14T10:18:08' },
      source: 'real',
    }),
    art('account_snapshot', {
      account_assets: { total_asset: 5176.1, market_value: 9265.26, positions: [{ symbol: 'ETHUSDT', extra: { unrealized_pnl: 30.47 } }], source: 'real', update_time: '2026-07-14T10:18:49' },
    }),
    art('execution_summary', {
      ...
      reconciliation: {
        ...
        symbols: [
          { symbol: 'ETHUSDT', status: 'SUCCEEDED', target: 2.599, filled: -1.794, filled_value: -3198.0, avg_price: 1782.73, before: 4.393, after: 2.599, moved: -1.794, drift: 0, attained_ratio: 1.0, reached: true },
        ],
      },
    }),
  ]
...
describe('buildExecutionDetail · 头条', () => {
  ...
  it('项失败按逐只成交结果计，不重复计入生命周期 ERROR 回声', () => {
    ...
    ;(summary.content.reconciliation as { symbols: unknown[] }).symbols.push({
      symbol: 'XRPUSDT', status: 'FAILED', target: null, filled: 0, filled_value: 0, avg_price: null, before: 974.2, after: 974.2, moved: 0, drift: 0, attained_ratio: null, reached: null,
    })
    ...
    // 2 条 ERROR 事件（XRP 决策 + 对账回声），但只有 1 个品种真失败
...
describe('buildExecutionDetail · 目标变化', () => {
  it('给出 last→curr 与算法参数摘要', () => {
    ...
    expect(tc?.rows).toEqual([{ symbol: 'ETHUSDT', curr: 0.9, last: 1.5 }])
...
describe('buildExecutionDetail · 逐只子链', () => {
  it('TCA 与订单树透传到 SymbolChain', () => {
    const { events, artifacts } = baseFixture()
    const summary = artifacts.find((a) => a.artifact_type === 'execution_summary')!
    const eth0 = (summary.content.reconciliation as { symbols: Record<string, unknown>[] }).symbols[0]!
    eth0.tca = { n_orders: 1, n_trades: 1, arrival_mid: 1782.66, slippage_bps: 0.36, liquidity: 'passive', fill_ratio: 1, fee: 0.64, fee_asset: 'USDT' }
    eth0.orders = [{ order_id: 'o1', side: 'sell', order_type: 'LIMIT', price: 1782.73, avg_price: 1782.73, volume: 1.794, filled_volume: 1.794, status: '已成交', client_order_id: 'lIww', trades: [{ price: 1782.73, volume: 1.794, value: 3198, time: '2026-07-14T10:18:26', fee: 0.64, fee_asset: 'USDT' }] }]
    const eth = buildExecutionDetail(events, artifacts).symbols[0]!
    expect(eth.tca?.liquidity).toBe('passive')
    expect(eth.tca?.slippage_bps).toBe(0.36)
    expect(eth.tca?.fee).toBe(0.64)
    expect(eth.orders).toHaveLength(1)
    expect(eth.orders[0]?.trades[0]?.fee).toBe(0.64)
  })

  it('减仓到位：方向/成交/均价/浮盈/耗时/无断点', () => {
    const { events, artifacts } = baseFixture()
    const eth = buildExecutionDetail(events, artifacts).symbols[0]!
    expect(eth.action).toBe('reduce')
    expect(eth.side).toBe('sell')
    expect(eth.filled).toBe(-1.794)
    expect(eth.filledValue).toBe(-3198.0)
    expect(eth.avgPrice).toBe(1782.73)
    expect(eth.reached).toBe(true)
    expect(eth.legSeconds).toBe(19)
    expect(eth.clientOrderId).toBe('lIww')
    expect(eth.pnlBefore).toBe(51.53)
    expect(eth.pnlAfter).toBe(30.47)
    expect(eth.broken).toBe(false)
  })
  ...
  it('撤销欠量：终态撤销 + reached=false → 断点变红并给原因', () => {
    ...
    const summary = artifacts.find((a) => a.artifact_type === 'execution_summary')!
    ;(summary.content.reconciliation as { symbols: unknown[] }).symbols.push({
      symbol: 'BTCUSDT', status: 'SUCCEEDED', target: 0.1243, filled: 0.0675, filled_value: 4197, avg_price: 62174, before: 0.0336, after: 0.1011, moved: 0.0675, drift: 0, attained_ratio: 0.81, reached: false,
    })
    const btc = buildExecutionDetail(events, artifacts).symbols.find((s) => s.symbol === 'BTCUSDT')!
    expect(btc.terminalStatus).toBe('已撤销')
    expect(btc.reached).toBe(false)
    expect(btc.broken).toBe(true)
    expect(btc.reason).toContain('撤')
  })
  ...
  it('受阻且已到位：订单腿 BLOCKED 但仓位在目标 → 不判失败、不算断点、原因翻中文', () => {
    ...
    const summary = artifacts.find((a) => a.artifact_type === 'execution_summary')!
    ;(summary.content.reconciliation as { symbols: unknown[] }).symbols.push({
      symbol: 'BNBUSDT', status: 'BLOCKED', target: 2.372859, filled: 0, filled_value: 0, avg_price: null, before: 2.37, after: 2.37, moved: 0, drift: 0, attained_ratio: 0.9988, reached: true,
    })
    const bnb = buildExecutionDetail(events, artifacts).symbols.find((s) => s.symbol === 'BNBUSDT')!
    expect(bnb.reached).toBe(true)
    expect(bnb.action).not.toBe('failed')
    expect(bnb.broken).toBe(false)
    expect(bnb.reason).toBe('受阻')
  })
  ...
  it('受阻且未到位：订单腿 BLOCKED 且仓位掉出目标 → 判失败并断点，原因给「受阻」', () => {
    ...
    const summary = artifacts.find((a) => a.artifact_type === 'execution_summary')!
    ;(summary.content.reconciliation as { symbols: unknown[] }).symbols.push({
      symbol: 'BNBUSDT', status: 'BLOCKED', target: 3, filled: 0, filled_value: 0, avg_price: null, before: 1, after: 1, moved: 0, drift: 0, attained_ratio: 0.33, reached: false,
    })
    const bnb = buildExecutionDetail(events, artifacts).symbols.find((s) => s.symbol === 'BNBUSDT')!
    expect(bnb.reached).toBe(false)
    expect(bnb.action).toBe('failed')
    expect(bnb.broken).toBe(true)
    expect(bnb.reason).toBe('受阻')
  })
  ...
  it('真失败且结果未知：决策 FAILED、reached=null → 判失败并断点，原因行不回声英文/「失败」', () => {
    ...
    const xrp = buildExecutionDetail(events, artifacts).symbols.find((s) => s.symbol === 'XRPUSDT')!
    expect(xrp.reached).toBeNull()
    expect(xrp.action).toBe('failed')
    expect(xrp.broken).toBe(true)
    // FAILED 与头部「失败」同义反复 → 原因行留空，不吐 FAILED/失败
    expect(xrp.reason).toBe('')
  })

  it('跳过：symbol_skipped → action=skipped + 原因 + 断点', () => {
    const { events, artifacts } = baseFixture()
    events.push(ev({ event_type: 'symbol_skipped', symbol: 'SOLUSDT', status: 'WARNING', reason_code: 'RISK.FORBIDDEN' }))
    const summary = artifacts.find((a) => a.artifact_type === 'execution_summary')!
    ;(summary.content.reconciliation as { symbols: unknown[] }).symbols.push({
      symbol: 'SOLUSDT', status: 'SKIPPED', target: 1, filled: 0, filled_value: 0, avg_price: null, before: 0, after: 0, moved: 0, drift: 0, attained_ratio: 0, reached: false,
    })
    const sol = buildExecutionDetail(events, artifacts).symbols.find((s) => s.symbol === 'SOLUSDT')!
    expect(sol.action).toBe('skipped')
    expect(sol.broken).toBe(true)
    expect(sol.reason).toBe('RISK.FORBIDDEN')
  })
})
...
describe('buildExecutionDetail · 脊柱与降级', () => {
  ...
  it('有减有增 → 拆成 阶段1 减仓 + 阶段2 开仓（阶段1 全成才跑）', () => {
    ...
    const summary = artifacts.find((a) => a.artifact_type === 'execution_summary')!
    ;(summary.content.reconciliation as { symbols: unknown[] }).symbols.push({
      symbol: 'SOLUSDT', status: 'SUCCEEDED', target: 10, filled: 10, filled_value: 1000, avg_price: 100, before: 0, after: 10, moved: 10, drift: 0, attained_ratio: 1, reached: true,
    })
    const spine = buildExecutionDetail(events, artifacts).spine
    const byKey = Object.fromEntries(spine.map((n) => [n.key, n]))
    expect(byKey.phase1?.label).toBe('阶段1 减仓') // ETH 减仓
    expect(byKey.phase2?.label).toBe('阶段2 开仓') // SOL 建仓
    expect(byKey.phase2?.detail).toContain('阶段1 全成才跑')
    expect(byKey.target?.detail).toBe('目标下调') // 目标快照里只有 ETH（下调）
  })
