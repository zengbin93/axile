    ...
    <>
      ...
      >
        <div className="flex h-14 flex-none items-center border-b border-line px-4 sm:px-5">
          <div id="directory-picker-title" className="text-[16px] font-[640]">选择目录</div>
        ...
        <form className="flex flex-none gap-2 border-b border-line px-4 py-3 sm:px-5" onSubmit={submitLocation}>
          ...
          <input ref={locationRef} value={location} onChange={(event) => setLocation(event.target.value)} placeholder="输入绝对路径" spellCheck={false} className="h-10 min-w-0 flex-1 rounded-[8px] border border-line bg-surface px-3 font-mono text-[13px] outline-none focus:border-accent" />
          <button type="submit" className="h-10 flex-none cursor-pointer rounded-[8px] border border-line px-4 text-[13px] text-ink-2">前往</button>
        </form>
        ...
        <div className="flex min-h-10 flex-none items-center gap-1 overflow-x-auto border-b border-line px-4 py-2 sm:px-5" aria-label="当前目录">
          {listing?.path ? directoryBreadcrumbs(listing.path).map((crumb, index, crumbs) => (
            <div key={crumb.path} className="flex flex-none items-center">
              ...
              <button type="button" className={`cursor-pointer rounded px-1.5 py-1 text-[12px] ${index === crumbs.length - 1 ? 'text-ink-1' : 'text-ink-2 hover:bg-fill'}`} onClick={() => void load(crumb.path)}>
                {crumb.label}
              </button>
            </div>
          )) : <span className="text-[12px] text-ink-3">本机磁盘</span>}
        </div>

        <div className="min-h-0 flex-1 overflow-y-auto px-2 py-2 sm:px-3" aria-busy={loading}>
          {error && <div className="px-3 py-4 text-[13px] text-warn">{error}</div>}
          {!error && loading && !listing && <div className="px-3 py-4 text-[13px] text-ink-3">正在读取目录…</div>}
          {!error && listing?.entries.length === 0 && <div className="px-3 py-4 text-[13px] text-ink-3">此目录没有子文件夹</div>}
